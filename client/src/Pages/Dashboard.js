...
const Dashboard = () => {
    ...
    return ( <
        ...
        <
        button className = "px-6 py-2 text-sm font-medium text-gray-800 bg-teal-400 hover:bg-teal-300 rounded-full shadow-lg transition duration-300" >
        See All <
        /button> <
        /Link> <
        /div> <
        ...
        tbody > {
            attendanceData.map((item, index) => ( <
                ...
                className = "hover:bg-gray-800 text-sm" >
                <
                td className = "px-4 py-2 border border-gray-700 text-center" > { index + 1 } < /td> <
                td className = "px-4 py-2 border border-gray-700 text-center" > { item.employeeName ? capitalize(item.employeeName) : "N/A" } <
                /td>

                { /* Format Login Time */ } <
                td className = { `px-4 py-2 border border-gray-700 text-center ${item.isOnTime ? 'text-green-500' : 'text-red-500'}` } >
                { formatTime(item.loginTime) } <
                /td>

                { /* Format Logout Time */ } <
                td className = "px-4 py-2 border border-gray-700 text-center" > { formatTime(item.logoutTime) } <
                /td> <
